...
// Create a Figma Paint (Color) Style
async function createPaintStyle(params) {
  if (!params || !params.name) {
    throw new Error("Missing required parameter: name");
  }
  if (!params.color || typeof params.color.r !== "number") {
    throw new Error("Missing required parameter: color {r, g, b}");
  }
  ...
  return {
    ...
    name: style.name,
    key: style.key,
    message: `Created color style "${style.name}"`,
  };
}

// Create a Figma Text Style
async function createTextStyle(params) {
  if (!params || !params.name) {
    throw new Error("Missing required parameter: name");
  }
  if (!params.fontSize || typeof params.fontSize !== "number") {
    throw new Error("Missing required parameter: fontSize");
  }
  ...
  if (params.lineHeightPx) {
    style.lineHeight = { value: params.lineHeightPx, unit: "PIXELS" };
  } else if (params.lineHeightPercent) {
    style.lineHeight = { value: params.lineHeightPercent, unit: "PERCENT" };
  }

  if (typeof params.letterSpacing === "number") {
    style.letterSpacing = { value: params.letterSpacing, unit: "PERCENT" };
  }
  ...
  return {
    ...
    name: style.name,
    key: style.key,
    message: `Created text style "${style.name}" (${params.fontSize}px ${fontFamily} ${fontStyle})`,
  };
}
...
async function renameNode(params) {
  ...
  return {
    ...
    oldName,
    name: node.name,
    message: `Renamed "${oldName}" → "${node.name}"`,
  };
}

// Move a node into a new parent (reparent)
async function reparentNode(params) {
  ...
  if (!parent) {
    throw new Error(`Parent node with ID ${params.parentId} not found`);
  }

  if (!("appendChild" in parent)) {
    throw new Error(`Node "${parent.name}" (${parent.type}) cannot contain children`);
  }

  const oldParentName = node.parent ? node.parent.name : "unknown";
  ...
  return {
    ...
    message: `Moved "${node.name}" from "${oldParentName}" into "${parent.name}"`,
  };
}
