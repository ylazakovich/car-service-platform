...
server.tool(
  ...
  "Create a new Figma Component node (◆) — use instead of create_frame when building reusable design system components",
  {
    x: z.number().describe("X position"),
    y: z.number().describe("Y position"),
    width: z.number().describe("Width of the component"),
    height: z.number().describe("Height of the component"),
    name: z.string().optional().describe("Component name, e.g. 'Chip/Tracking'"),
...
server.tool(
  ...
  {
    name: z.string().describe("Style name, e.g. 'Color/Primary' or 'Color/Bg/Surface'"),
    color: z.object({
  ...
  async ({ name, color }: any) => {
    try {
      const result = await sendCommandToFigma("create_paint_style", { name, color }) as any;
      return {
...
server.tool(
  ...
  {
    name: z.string().describe("Style name, e.g. 'Heading/H1' or 'Body'"),
    fontSize: z.number().describe("Font size in pixels"),
    fontFamily: z.string().optional().default("Inter").describe("Font family name"),
    fontStyle: z.string().optional().default("Regular").describe("Font style: Regular, Bold, Semi Bold, Medium, Italic, etc."),
    lineHeightPx: z.number().optional().describe("Line height in pixels"),
    lineHeightPercent: z.number().optional().describe("Line height as percent of font size (e.g. 120 = 120%)"),
    letterSpacing: z.number().optional().describe("Letter spacing in percent (e.g. 0 = normal, 5 = 5%)"),
  },
  async (params: any) => {
    try {
      const result = await sendCommandToFigma("create_text_style", params) as any;
      return {
...
server.tool(
  ...
  async ({ nodeId, name }: any) => {
    try {
      const result = await sendCommandToFigma("rename_node", { nodeId, name }) as any;
      return {
...
server.tool(
  ...
  "Move a node into a different parent frame or group — essential for building proper component internals",
  {
    ...
    parentId: z.string().describe("ID of the destination parent frame/group/component"),
    index: z.number().optional().describe("Position index within the parent's children (0 = front). Omit to append at end."),
  },
  async ({ nodeId, parentId, index }: any) => {
    try {
      const result = await sendCommandToFigma("reparent_node", { nodeId, parentId, index }) as any;
      return {
