...
if command -v lsof >/dev/null 2>&1; then
  if lsof -nP -iTCP:"${FRONTEND_DEV_PORT}" -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    echo "Often the prod stack (start.sh) or another app is using this port. Stop it, or pick a free port in .env, e.g.:" >&2
...
echo "Prod-like stack: bash scripts/start.sh"
