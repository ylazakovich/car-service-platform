...
| [latest/pyramid-snapshot.json](./latest/pyramid-snapshot.json) | Machine-readable latest snapshot (same source as above). |
| [latest/pyramid-quality-gates.json](./latest/pyramid-quality-gates.json) | Advisory quality-gate result from `pyramid-check` (warnings list; workflow still exits 0). |
